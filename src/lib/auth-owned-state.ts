...
/**
 * Publishes auth invalidation synchronously, before the async database runtime finishes switching.
 * Only semantic owner/resolution transitions advance the volatile generation. Token refreshes for
 * the same resolved owner keep first-paint caches and clean session baselines intact.
 */
...
export function registerAuthOwnedVolatileInvalidator(invalidate: () => void) {
	...
	return () => {
		volatileInvalidators.delete(invalidate);
	};
}
